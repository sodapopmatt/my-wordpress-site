...
class Helper {
	...
	/**
	 * Get all post types and their taxonomies/terms.
	 *
	 * @return array
	 */
	...
	public static function schedule_import_campaigns( $params ) {
		...
		switch ( $frequency ) {
			case 'hourly':
				...
				break;
			case 'daily':
				$specific_time = $params['schedule_settings']['time'];
				$tomorrow      = gmdate( 'Y-m-d', strtotime( 'tomorrow' ) );
				$timestamp     = strtotime( "$tomorrow $specific_time UTC" );
				$interval      = DAY_IN_SECONDS;
				break;
			case 'weekly':
				$specific_day  = $params['schedule_settings']['specific_day'];
				$specific_time = $params['schedule_settings']['time'];
	...
	/**
	 * Filter the campaign content (patched for empty/null inputs).
	 *
	 * @param mixed $content The campaign content.
	 * @return string
	 */
	public static function filter_campaign_content( $content ) {

		// ✅ If $content is empty or not a string, skip safely.
		if ( empty( $content ) || ! is_string( $content ) ) {
			// Optional logging (uncomment to debug skipped posts)
			// error_log('[Beehiiv Import] Skipped campaign with empty content.');
			return '';
		}
		...
		// Remove extra whitespace safely.
		$content = preg_replace( '/^\s+|\n|\r|\s+$/m', '', $content );

		// ✅ Prevent DOMDocument crash for truly empty strings.
		if ( trim( $content ) === '' ) {
			return '';
		}
		...
		try {
			...
			error_log('[Beehiiv Import] Failed to loadHTML: ' . $e->getMessage());
			return '';
		}
		libxml_clear_errors();

		// Remove <div id="web-header"> if present.
		$web_header_tag = $doc->getElementById( 'web-header' );
		if ( $web_header_tag && $web_header_tag->parentNode ) {
